...
ggplot(county_data) +
  ...
  geom_histogram(bins = 10, fill = "steelblue", alpha = 0.8) +
  labs(
...
ggplot(county_data) +
  ...
  labs(
    title = "Population Distribution of Pennsylvania Counties",
    x = "Total Population",
    y = "Number of Counties"
  ) +
  theme_minimal() +
...
# Box plot to identify population outliers
ggplot(county_data) +
  aes(y = total_popE) +
...
# Identify outlier counties using IQR
...
# Find top 3 and bottom 3 counties by population
most_populous <- county_data %>%
  arrange(desc(total_popE)) %>%
  select(county_name, total_popE) %>%
  head(3)

least_populous <- county_data %>%
  arrange(total_popE) %>%
  select(county_name, total_popE) %>%
  head(3)

print("3 most populous counties:")
most_populous

print("3 least populous counties:")
least_populous
...
ggplot(county_data) +
  ...
  geom_point(color = "red", alpha = 0.5) +   # 红色点，50%透明
  geom_smooth(method = "loess", se = TRUE, color = "yellow") +  # LOESS 平滑趋势线
  theme_dark() +  # 深色主题
  scale_y_continuous(labels = dollar) +  # Y 轴美元格式
  labs(
    title = "Relationship between Median Age and Median Household Income",
    x = "Median Age (years)",
    y = "Median Household Income ($)"
  )
```
...
```{r}
# Step 1: Calculate MOE percentage for median age
county_data <- county_data %>%
  mutate(age_MOE_pct = (median_ageM / median_ageE) * 100)

# Step 2: Scatter plot - Population vs Age MOE percentage
ggplot(county_data) +
  aes(x = total_popE, y = age_MOE_pct) +
  geom_point(color = "purple", size = 2) +
  geom_hline(yintercept = 5, color = "blue", linetype = "dashed") +
  labs(
    title = "Population vs Age MOE Percentage",
    x = "Total Population",
    y = "Median Age MOE (%)"
  ) +
  theme_classic() +
...
county_data <- county_data %>%
  mutate(
    pop_category = case_when(
      total_popE < 50000 ~ "Small",
      total_popE >= 50000 & total_popE < 200000 ~ "Medium",
      total_popE >= 200000 ~ "Large"
    )
  )

# Step 4: Boxplot comparing MOE across categories
ggplot(county_data) +
  aes(x = pop_category, y = age_MOE_pct, fill = pop_category) +
  geom_boxplot(alpha = 0.7) +
  labs(
    title = "Age MOE Percentage by Population Category",
    x = "Population Category",
    y = "Median Age MOE (%)"
  ) +
  theme_classic()
